...
A GNN encoder maps (X,A) to Z and decodes edges/features; VGAE uses q(Z|X,A).
...
A GNN encoder maps (X,A) to Z and decodes edges/features; VGAE uses q(Z|X,A). Latent dimensions and learned weights are not direct measurements.

## Evidence

Kipf & Welling (2016). See `SOURCES.md`; no project dataset was fitted.

...
This host supports infrastructure tests and modest CPU baselines. CUDA-specific upstream environments cannot run natively. Apple Metal support does not imply that a repository supports MPS. No credentials or device identifiers are recorded.

User-provided environment context confirms access to Google Colab through local Colab extensions. Colab is the preferred available execution environment when GPU, larger RAM or substantial compute is beneficial. Its runtime type, GPU model, CUDA version and installed software remain `UNKNOWN` until captured from each allocated runtime.

...
Use reported/default parameters first, then a documented reasonable search with the same budget where feasible. Record sensitive parameters such as latent dimension, neighbors, graph weights, KL/contrastive coefficients, learning rate, epochs and clustering resolution. Do not tune against the final biological test set. A method-specific native requirement is recorded rather than silently removed.

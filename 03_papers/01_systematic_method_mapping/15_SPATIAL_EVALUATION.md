...
Separate cluster agreement from spatial coherence. Planned spatial summaries include neighbor-label agreement, boundary-aware preservation, graph connectivity and spatial autocorrelation with an explicit null and coordinate units. Smooth clusters can merge adjacent biological regions, so spatial coherence receives no automatic credit without region/marker checks.
